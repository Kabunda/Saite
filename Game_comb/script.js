class MathSprint {
    constructor() {
        ...
        this.state = {
            timeLeft: 180,
            score: 0,
            highscore: JSON.parse(localStorage.getItem('highscore_mult')) || { 
                value: 0, 
                date: "не установлен", 
                name: "неизвестен" 
            },
    ...
    generateProblem() {
        ...
        let a, b;
        const multipliers = [5, 11, 8, 17, 35];
        switch(difficulty) {
            case 'easy':
                a = multipliers[this.getRandom(0, 4)];
            ...
            case 'medium':
                ...
                break;
            case 'hard':
                a = multipliers[this.getRandom(2, 4)];
                b = this.getRandom(3, 20);
                break;
        }
        if (mode == "blackjack") {
        ...
        this.elements.problem.textContent = `${b} x ${a}`;
    ...
    resetGame() {
        ...
        if (finalScore > this.state.highscore.value) {
            const recordData = {
                ...
                localStorage.setItem('highscore_mult', JSON.stringify(recordData));
